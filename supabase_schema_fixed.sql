-- 说明：
-- 1) 这份脚本只操作 public schema（你的业务表），不会引用 supabase_migrations.*
-- 2) 可重复运行：建表使用 IF NOT EXISTS；示例数据使用固定 UUID + ON CONFLICT DO NOTHING
-- 3) 默认只开放 “读取” 给 anon/authenticated，便于前端先跑通展示
...
create table if not exists public.matches (
  ...
  -- 让 (user_a, user_b) 永远按字典序存放，避免 A-B / B-A 两条重复
...
-- 先不做写权限：只给前端展示用的读权限（anon/authenticated 都可读）
do $$
begin
  if not exists (
    select 1
...
-- 验证（可选）：运行后应该能查到 5 行
-- select id, nickname, gender, age from public.profiles order by created_at asc;
